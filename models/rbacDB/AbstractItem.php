...
abstract class AbstractItem extends ActiveRecord
{
	...
	public static function removeChildren($parentName, $childrenNames)
	{
		...
		foreach ($childrenNames as $childName)
		{
			Yii::$app->db->createCommand()
				->delete(Yii::$app->getModule(\Yii::$app->user->moduleAliasName)->auth_item_child_table, ['parent' => $parentName, 'child' => $childName])
				->execute();
		}
	...
	{
		return Yii::$app->getModule(\Yii::$app->user->moduleAliasName)->auth_item_table;
	}
	...
	{
		return parent::find()->andWhere([Yii::$app->getModule(\Yii::$app->user->moduleAliasName)->auth_item_table . '.type'=>static::ITEM_TYPE]);
	}
