...
class User extends UserIdentity
{
	...
	public static function assignRole($userId, $roleName)
	{
		try
		{
			Yii::$app->db->createCommand()
				->insert(Yii::$app->getModule(\Yii::$app->user->moduleAliasName)->auth_assignment_table, [
					'user_id' => $userId,
					'item_name' => $roleName,
					'created_at' => time(),
				])->execute();
	...
	public static function revokeRole($userId, $roleName)
	{
		$result = Yii::$app->db->createCommand()
			->delete(Yii::$app->getModule(\Yii::$app->user->moduleAliasName)->auth_assignment_table, ['user_id' => $userId, 'item_name' => $roleName])
			->execute() > 0;
	...
	*/
	...
	{
		return Yii::$app->getModule(\Yii::$app->user->moduleAliasName)->user_table;
	}
	...
	*/
	...
	public function getRoles()
	{
		return $this->hasMany(Role::className(), ['name' => 'item_name'])
			->viaTable(Yii::$app->getModule(\Yii::$app->user->moduleAliasName)->auth_assignment_table, ['user_id'=>'id']);
	}
